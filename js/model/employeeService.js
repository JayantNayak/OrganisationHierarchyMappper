 ...
 function employeeService(){
	...
	this.showMangerSelectList = function( selectId,defaultValue){
	 ...
	 this.editEmployeeDetails = function(employeeName){
		 ...
		 var userDb=openDatabase(constantVal.databaseName, constantVal.databaseVersion,message.databaseDescription, constantVal.databaseSize);
				userDb.transaction(function(tx){
					//tx.executeSql("SELECT * FROM  empTable where empName = '"+employeeName+"'",[],function(tx,results){
...
function parent(current){
	...
	if(current != constantVal.rootEmp)
	{
			var userDb=openDatabase(constantVal.databaseName, constantVal.databaseVersion,message.databaseDescription, constantVal.databaseSize);
				...
				userDb.transaction(function(tx){
						...
						tx.executeSql('SELECT empName, manager from ' +constantVal.tblOrgName+ ' WHERE empName = ?',[current],function(tx,results){
							...
							//current=results.rows.item(0).manager;
		...
		//alert(circularReferenceUtility);
		constantVal.circularReferenceUtil = circularReferenceUtility;
		//alert("hhhhhhhhhhhhhhhhhhhhhh");
		//alert(constantVal.circularReferenceUtil);
		//alert(circularReferenceUtility);
		//var f = constantVal.circularReferenceUtil.indexOf(tempName) == -1;
		//var g = constantVal.circularReferenceUtil[tempName] ;
		 //alert("  condition --> "+f +" array --> "+ g);
		// console.log("  condition --> "+f +" array --> "+ g);
		...
		if( constantVal.circularReferenceUtil.indexOf(tempName) == -1)
		{
			var userDb=openDatabase(constantVal.databaseName, constantVal.databaseVersion,message.databaseDescription, constantVal.databaseSize);
				...
				userDb.transaction(function(tx){
					...
					//update the dependencies associated with the current record to be edited
					//here name is the primary key so if it gets edited then update all the depenedent records
					/*
					if(tempName!=tempPreviousName ){
						console.log("-------------------updating dependecies------------------");
						tx.executeSql('Select * from  '+constantVal.tblOrgName +' WHERE manager = ?',[tempPreviousName],function(tx,results){
							//console.log("dependecies length ->" +results.rows.length);
							alert("dependecies length ->" +results.rows.length);
						});
					}
					*/
